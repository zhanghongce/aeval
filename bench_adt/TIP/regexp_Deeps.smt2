...
(assert
  (forall ((x R))
    (= (deeps x)
      (ite
        (is-Star x) (deeps (Star_0 x))
        (ite
          (is-Seq x)
          (ite
            (and (eps (Seq_0 x)) (eps (Seq_1 x)))
            (Plus (deeps (Seq_0 x)) (deeps (Seq_1 x)))
            (Seq (Seq_0 x) (Seq_1 x)))
          (ite
            (is-Plus x) (Plus (deeps (Plus_0 x)) (deeps (Plus_1 x)))
            (ite (is-Atom x) (Atom (Atom_0 x)) (ite (is-Eps x) Nil Nil))))))))
(check-sat)
